...
管理员也可以在聊天中使用 `/nradio_kb_sync` 立即同步，使用 `/nradio_kb_status` 查看最近一次结果。

## 权限边界

插件对 GitHub 只需要读取权限，对 AstrBot 只操作配置中选中的知识库。GitHub 审核、PR 合并和知识内容修改仍在 `nradio-platform` 中完成，插件本身不会生成、审核或改写知识。
